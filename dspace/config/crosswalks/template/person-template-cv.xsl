...
<xsl:stylesheet version="1.1"
    ...
    <xsl:template match="person">   
        <fo:root
            ...
            <xsl:if test="embedded-pdfs/embedded-pdf">
               <xsl:for-each select="embedded-pdfs/embedded-pdf">
                   <xsl:variable name="uuid" select="self::node()"/>
                   <xsl:variable name="pdfPath" select="concat('file:',$imageDir,'/',$uuid)" />
