...
<xsl:stylesheet version="1.1"
  ...
  <!-- VARIABLES -->	
	...
	<xsl:param name="leftMarginText" select="'3mm'"/>
			...
			<fo:layout-master-set>
				...
				<fo:simple-page-master master-name="A4-process-events"
					...
					<fo:region-body border-before-width="0"> <!-- according to the standard the width has be 0, 
																												this affectively avoids border lines to be implemented (unless the standard is ignored which DSpace does not do), 
  ...
  <!-- USED TEMPLATES -->	
	...
	<xsl:template name = "entity-name" >
		...
		<fo:block margin-left="{$leftMarginText}"
							margin-top="5mm">
	...
	<xsl:template name = "print-value" >
	  ...
	  <xsl:if test="$value">
		  <fo:block font-size="{$mainFontSize}" 
			        	margin-top="2mm">
